...
if ( ! class_exists( 'Storefront_NUX_Admin' ) ) :
	...
	class Storefront_NUX_Admin {
		...
		public function dismiss_nux() {
			if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['nonce'] ) ), 'storefront_notice_dismiss' ) || ! current_user_can( 'manage_options' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				die();
			}
		...
		public function redirect_customizer() {
			...
			if ( ! empty( $_REQUEST['homepage'] ) && 'on' === sanitize_text_field( wp_unslash( $_REQUEST['homepage'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( ! empty( $_REQUEST['products'] ) && 'on' === sanitize_text_field( wp_unslash( $_REQUEST['products'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$tasks[] = 'products';
			}
